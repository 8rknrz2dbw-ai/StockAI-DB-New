...
MARKETS = ["台北一", "台北二", "三重", "板橋", "宜蘭", "桃園", "台中", "豐原",
           ...
           "埔里", "東勢", "員林", "二林"]
# 上表為候選短名（用短名做 API 過濾）。實際能否抓到取決於該市場當期是否有蔬果成交、
# 且 API「市場名稱」需相符；對不到／無成交者由主流程自動略過、不影響其他市場（見 fetch_market_all）。
# 沙盒擋 data.moa.gov.tw，市場名以 GitHub Actions 實跑驗證（workflow_dispatch）。
