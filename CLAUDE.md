...
- 版本號在 `index.html` 的 `const VERSION`（頁尾顯示「精準務農 vX.Y.Z」，淺灰置中）。
  ...
  目前版本 **v1.7.0**（我的品項「細部品種可各別勾選」：watch 項目可為母作物或「母作物␟品種」(VSEP)，
  勾品種→看板出現該品種卡「母·品種」直接進 K線；搜尋/自動完成也吃品種名。颱風「追蹤中」跑很遠(>600km)或
  遠離就不再顯示(後端 tracking＝非威脅×非遠離×近；前端 showTracking 同條件)，之後又逼近自動恢復。
  前一版 v1.6.0：頁首固定置頂＋瀏海、看板市場下拉同步、我的品項整齊清單）。
