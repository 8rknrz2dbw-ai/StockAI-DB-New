...
- 版本號在 `index.html` 的 `const VERSION`（頁尾顯示「精準務農 vX.Y.Z」，淺灰置中）。
  ...
  目前版本 **v1.10.0**（我的品項「類型小標」全面校準＋水果也分小標：蔬菜 8 類(新增香辛香草類、LEAF_FIRST 修正
  甘薯葉/薯蕷/扁蒲等誤分)、水果新增 7 類(柑橘/瓜/熱帶/漿果/核果/仁果/柿)，正則以線上實際作物名校準到 0 誤分；
  後端 fetch_prices 市場改動態探索補齊合作社/農會市場。前一版 v1.9.0：字級預設中、我的品項折疊、左緣右滑上一頁）。
