# CLAUDE.md — 農商雷達（原「豐雨無阻」／「二崙葉菜行情室」）＋ 颱風搶收預判系統
...
index.html                    單頁多分頁：行情看板／田區搶收／採收預判／跨市場比價(compare)。
                              ...
                              ＋windy 雷達 iframe＋NCDR/windy 連結。
                              有品種的作物(如鳳梨)點卡片先下拉選品種(全部綜合/金鑽/牛奶…)再進 K線。
                              品項圖示為自繪 SVG 精緻插圖(CROP_SVG，零版權；政府農試所素材為「政府資料開放
                              授權」需標示來源且無逐作物照片包故不採)：看板卡/品種下拉/介紹彈窗皆用，品種
                              (愛文紅/金煌黃/凱特綠)有專屬色調，查不到才退回 emoji(cropIconHTML/cropSvg)。
                              頁首：實際·示範 badge 靠左、其右接更新日期、⚙️最右。App 名農商雷達見 PWA/頁尾。
                              全站兩指可縮放放大檢視、放開回彈原狀(純視覺 transform scale，不改版面)。
                              全國定位：颱風侵襲機率依「所在縣市」中心點×颱風軌跡計算(regionInvasion，不寫死雲林)；
                              文案改「全台蔬果」。設定分四類(顯示與身分/行情看板/天氣與田區/關於)；我的品項有全選/
                              全部關閉＋已選計數；田區表單附「Google Earth 量田」教學(量㎡→填入，海拔對照地勢)。
                              ...
                              設定內「關於 App」(上架用)：意見回饋(mailto)/常見問題(FAQ modal)/隱私權政策
                              (privacy.html)/使用條款(terms.html)/分享App(navigator.share→剪貼簿退回)。
...
- 版本號在 `index.html` 的 `const VERSION`（頁尾顯示「農商雷達 vX.Y」，淺灰置中）。
  採語意化版號 SemVer-lite：`v1.0` 為「巡田水」重啟基準、`v3.0` 為改名「農商雷達」的重啟基準，
  每次前端有感更新遞增次版號，大改版/改名時進位主版號。刻意不用日期式版號，以免和右上「更新：<資料時間>」混淆。
