...
scripts/
  fetch_prices.py             抓農業部「農產品交易行情」公開資料（免金鑰，近 2 年）→
                              prices_index.json（markets/crop_map/categories/latest）+ prices/NN.json。
                              VEG_MAP(蔬菜)+FRUIT_MAP(水果)，categories 供前端分類切換/比價分組。
                              解析時跳過 avg<=0（休市/未成交）→ 各市場檔只留真的有交易的品項
                              （各地種類自然不同：葉菜市場只有葉菜、大市場才有整排水果）。
                              latest 取「最近一個 avg>0」的日期（_latest_valid）。
