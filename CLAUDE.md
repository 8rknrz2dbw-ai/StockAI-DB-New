...
- 版本號在 `index.html` 的 `const VERSION`（頁尾顯示「精準務農 vX.Y.Z」，淺灰置中）。
  ...
  目前版本 **v1.6.0**（頁首固定置頂＋瀏海安全區(header sticky、--hdr-h 讓看板子列貼其下)；行情看板市場改「下拉選單」
  (mktLabelSel)與設定同步(switchMarket)；設定我的品項改整齊清單、品種各自獨立小標(.pickrow/.prvar，不再擠成一串)。
  前一版 v1.5.0：點卡片進 K線＋「ⓘ說明」鈕、搜尋自動完成、K線查價線橫滑才顯示）。
