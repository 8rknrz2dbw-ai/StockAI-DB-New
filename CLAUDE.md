...
設定預設：字體 `vs_fs` 預設「小(s)」、果菜市場 `DEFAULT_MARKET="西螺"`、天氣地區 `vs_region` 預設「雲林縣」。
...
```
index.html                    單頁多分頁：行情看板／採收預判／田區巡查（farmer 單一身分，無角色切換、無比價分頁）。
                              （「田區巡查」原名「田區搶收」，2026-07 改名；內部 data-t 仍為 harvest，決策標籤仍用「立即/建議搶收」。）
                              字體 s/m/l/xl（預設 s 小）。
                              行情看板為「大卡列表」：每卡顯示品項名＋均價＋漲跌＋偏貴/偏便宜標籤，全站可切
                              公斤/台斤(1台斤=0.6kg，S.unit)。看板頂有搜尋框(跨市場全部品項、不分蔬果)＋市場標示；
                              預設看板優先列常見葉菜(COMMON_VEG)再補量大者。
                              ⚑ 2026-07（v1.3.0）品項互動改版：**點卡片＝開「品項說明」彈窗**(openCropInfo：挑選要訣＋
                              「用 Google 查詢『品項』實際照片」鈕)。單一品項→直接開說明；多品項→先展開各品種
                              (無「全部綜合」)、點某品種→開該品種說明。**品項圖示全面移除**(蔬菜 emoji/水果放大鏡皆刪)、
                              說明彈窗大圖示亦刪。手繪 CROP_SVG/放大鏡 SEARCH_SVG/cropIconHTML/wireIcon 已刪除。
                              cropIcon(emoji) 僅保留給「採收預判」推薦清單。
                              註：K線詳細頁(openDetail/renderDetail，日/週/月·2年 走勢＋逐日明細＋農曆)程式仍保留，
                              但**已無入口**(改點卡片開說明)；若要恢復價格走勢，把 tapCrop/品種點擊改回 openDetail 即可。
...
- 版本號在 `index.html` 的 `const VERSION`（頁尾顯示「精準務農 vX.Y.Z」，淺灰置中）。
  ...
  目前版本 **v1.3.0**（品項互動改版：點卡片開品項說明＋Google 查圖、移除所有品項圖示、多品項去掉「全部綜合」、
  K線詳細頁保留但無入口；另換用戶提供的 App 圖示）。
