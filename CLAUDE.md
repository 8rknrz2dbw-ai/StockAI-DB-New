...
index.html                    單頁多分頁：行情看板／田區搶收／採收預判／跨市場比價(compare)。
                              ...
                              品項圖示：蔬菜為自繪 SVG 精緻插圖(CROP_SVG，零版權)；水果品種多、插圖難精準→改用
                              放大鏡圖示，點了直接開 Google 圖片搜尋看實照(cropSearchURL，零版權不自存圖，
                              cropCat==="水果"時 cropIconHTML 回 SEARCH_SVG、wireIcon 直接 window.open)。蔬菜介紹
                              彈窗另有「用 Google 搜尋實際照片」鈕(ciSearch)。政府農試所素材為「政府資料開放授權」
                              需標示來源且無逐作物照片包故不採。查不到插圖才退回 emoji。
